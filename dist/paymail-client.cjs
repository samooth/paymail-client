...
class Http {
  ...
  async _basicRequest(url, options = {}) {
    ...
    return this.fetch(url, _objectSpread2(_objectSpread2({}, options), {}, {
      credentials: 'omit',
...
class EndpointResolver {
  constructor(dns = null, fetch) {
    this.dnsClient = new DnsClient(dns, new DnsOverHttps(fetch, {
      baseUrl: 'https://bsv.direct/dns-query'
    }));
  ...
  async getOrdinalAddressUrlFor(aPaymail) {
    ...
    const apiDescriptor = await this.getApiDescriptionFor(domain);
    const addressUrl = apiDescriptor.capabilities.ordinalPayment.replace('{alias}', alias).replace('{domain.tld}', domain);
    return addressUrl;
  }
  ...
  async fetchApiDescriptor(domain, port) {
    ...
    const wellKnown = await this.http.get(`${protocol}://${domain}${requestPort}/.well-known/bsvalias`, {
      headers: {
        accept: 'application/dns-json'
      }
    });
